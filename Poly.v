...
Fixpoint split {X Y : Type} (l : list (X*Y))
               : (list X) * (list Y) :=
  match l with
  | (x, y)::l' => let (x', y') := split l' in (x::x', y::y')
  | [] => ([], [])
  end.
...
Lemma map_concat_distrib : forall (X Y : Type) (f : X -> Y) (l l1 : list X),
  map f (l ++ l1) = (map f l) ++ (map f l1).
Proof.
  intros. induction l.
  - reflexivity.
  - simpl. rewrite IHl. reflexivity.
...
Proof.
  ...
  induction l.
  - reflexivity.
  - simpl. rewrite <-IHl. apply map_concat_distrib.
Qed.
(** [] *)
...
Definition exp (n m : nat) : nat :=
  fun X f => m (X -> X) (fun cf => n X cf) (one X f).
