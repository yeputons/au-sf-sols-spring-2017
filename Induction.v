...
Lemma mult_def_r : forall m n : nat,
  m * (S n) = m+ m * n.
Proof.
  intros.
  induction m.
...
Proof.
  ...
  - simpl. rewrite <-IHn. rewrite mult_def_r. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros.
  induction p.
  - rewrite plus_O_n. assert (H': 0+m=m). { reflexivity. } rewrite H'. rewrite H. reflexivity.
  - rewrite plus_Sn_m. assert (H': S p + m = S (p + m)). { rewrite plus_Sn_m. reflexivity. } rewrite H'. simpl. rewrite IHp. reflexivity.
Qed.
...
Proof.
  ...
  - rewrite mult_def_r. rewrite mult_def_r. rewrite mult_def_r.
    rewrite IHp.
    rewrite plus_assoc.
    rewrite plus_assoc.
