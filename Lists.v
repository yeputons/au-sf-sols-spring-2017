...
Fixpoint count (v:nat) (s:bag) : nat :=
  match s with
  | nil => 0
  | cons x l => (count v l) + (if (beq_nat x v) then 1 else 0)
  end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  ...
  | nil => nil
  | cons x s =>
    if (beq_nat v x) then s
    else cons x (remove_one v s)
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | nil => true
  | cons x s1 =>
    if beq_nat (count x s2) 0 then
      false
    else
      subset s1 (remove_one x s2)
  end.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | nil, nil => true
  | cons x l1, cons y l2 => beq_nat x y && beq_natlist l1 l2
  | _, _ => false
  end.
...
Proof.
  ...
  simpl.
  remember (count 1 s).
  assert (n + 1 = 1 + n). apply plus_comm.
  rewrite H.
  assert (1 + n = S n). apply plus_1_l.
  rewrite H0.
  reflexivity.
Qed.
...
Proof.
  ...
  - simpl. reflexivity.
  - destruct n.
    + assert (count 0 (0 :: s) = S (count 0 s)). simpl. apply plus_comm.
      rewrite H.
      assert (count 0 (remove_one 0 (0 :: s)) = count 0 s). auto.
      rewrite H0.
      apply ble_n_Sn.
    + simpl. 
      assert (forall x, x + 0 = x). intros. apply plus_comm.
      rewrite H. rewrite H. apply IHs.
Qed.
(** [] *)
...
Proof.
  intros. destruct x. simpl. apply beq_nat_refl.
Qed.
(** [] *)
...
Proof.
  intros.
  simpl.
  assert (true = beq_id x x). apply (beq_id_refl x).
  rewrite <-H.
  reflexivity.
Qed.
(** [] *)
